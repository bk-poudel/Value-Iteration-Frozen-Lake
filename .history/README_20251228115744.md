...
This repo demonstrates value iteration on the classic Frozen Lake problem from **Gymnasium** using two example entry points:
- `value_iteration.py` — runs value iteration on a small custom map with the default (slippery) dynamics.
...
```
Each run will print "Value Table Learned for Custom Map!" once the value function converges and then render an episode in a separate window.
## Customising the Map
Both scripts expose a `MY_MAP` constant near the top of the file. You can replace it with any rectangular grid made from the following characters:
